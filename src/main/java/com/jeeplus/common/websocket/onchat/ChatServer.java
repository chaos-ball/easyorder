...
public class ChatServer extends WebSocketServer {
  ...
  public void onMessage(WebSocket conn, String message) {
    ...
    if (null != message && message.startsWith(Constant._online_user_)) {// 用户上线
      ...
      for (ChatHistory c : list) {
        ...
        ChatServerPool.sendMessageToUser(conn,
            sender + Constant._msg_ + c.getUserid2() + Constant._msg_ + c.getMsg() + Constant._msg_
                + UserUtils.getByLoginName(userid1).getPhoto() + Constant._msg_ + c.getType()
                + Constant._msg_ + UserUtils.getByLoginName(userid1).getName() + Constant._msg_
                + c.getCreateDate().getTime());// 向所某用户发送消息
        c.setStatus("1");// 标记为已读
    ...
    if (null != message && message.contains(Constant._msg_)) {// 发送消息
      ...
      if ("group".equals(type)) {// 如果是群聊
        ...
        for (LayGroupUser lgUser : layGroupUserlist) {
          ...
          chat.setType("group");
          if (toUserConn != null) {
            message = groupId + Constant._msg_ + lgUser.getUser().getLoginName() + Constant._msg_
                + msg + Constant._msg_ + avatar + Constant._msg_ + type + Constant._msg_
                + fromUsername + Constant._msg_ + datatime;
  ...
  public void userjoin(String user, WebSocket conn) {
    ...
    // String joinMsg = "{\"from\":\"[系统]\",\"content\":\""+user+"上线了\",\"timestamp\":"+new
  ...
  public void userLeave(WebSocket conn) {
    ...
    // String joinMsg = "{\"from\":\"[系统]\",\"content\":\""+user+"下线了\",\"timestamp\":"+new
    // Date().getTime()+",\"type\":\"message\"}";
